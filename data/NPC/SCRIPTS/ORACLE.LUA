...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
  		selfSay('Hello ' .. cname .. '! Are you ready to face your destiny and save the world of Titanium?')
  		talk_state = 0
  		focus = cid
  	...
  	elseif focus == cid then		talk_start = os.clock()
  		if talk_state == 0 then			if msgcontains(msg, 'yes') then		-- wanna go to mainland
  				...
  				if level >= mainlevel then
 					...
 					talk_state = 1
  					...
  					selfSay('Sorry, you need level ' .. mainlevel .. ' to go to the mainworld.')
 					talk_state = 0
  				end
 			else
 		...
 		elseif talk_state == 2 then		-- confirming vocation
 			if msgcontains(msg, 'yes') then
 				selfSay('Great! I can send you to The Titanium. Want to go there? tace care we will pass trought the other dimension on the way to the Titanium')
 				talk_state = 3
 			else
 		...
 		elseif talk_state == 3 then		-- telling city name
 			if msgcontains(msg, 'titanium') then

				
				if vocation == 1  then
 				selfSay('Good luck, young adventurer!')
 				setPlayerVocation(cid,vocation)
 				selfSay('/send ' .. cname .. ', 135 282 14')
 			setPlayerMasterPos(cid,135,282,14)
 			...
 			setPlayerMasterPos(cid,135,282,14)
				...
				elseif vocation == 3  then
 				selfSay('Good luck, young adventurer!')
 				setPlayerVocation(cid,vocation)
 				selfSay('/send ' .. cname .. ', 159 282 14')
 			setPlayerMasterPos(cid,159,282,14)
				

				elseif vocation == 4   then
 				selfSay('Good luck, young adventurer!')
 				setPlayerVocation(cid,vocation)
 				selfSay('/send ' .. cname .. ', 181 282 14')
 			setPlayerMasterPos(cid,181,282,14)
				end
 				...
 				selfSay('Sorry, there is no such world.')
 				talk_state = 3
 			end
  		end
