attackType = ATTACK_NONE
 ...
 function onCast(cid, creaturePos, level, maglv, var)
    ...
    -- n = tonumber(var)   -- try to convert it to a number
    	...
    	-- else
    		--UltimateHealingObject.minDmg = (level * 2 + maglv * 3) * 1.5
    		--UltimateHealingObject.maxDmg = (level * 2 + maglv * 3) * 1.8
 UltimateHealingObject.minDmg = (level * 3 + maglv * 2) * 5
 if UltimateHealingObject.minDmg < 250 then
 	UltimateHealingObject.minDmg = 250
 end
 
 UltimateHealingObject.maxDmg = (level * 3 + maglv * 2) * 7
