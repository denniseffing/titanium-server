...
int Monster::onThink(int &newThinkTicks) {
    bool yelled = false;
    for (YellingSentences::iterator ysIt = mType->yellingSentences.begin();
         ysIt != mType->yellingSentences.end(); ++ysIt) {
    ...
    if (state != STATE_IDLE && state != STATE_TARGETNOTREACHABLE && !(isSummon() && hasLostMaster)) {
        if (updateMovePos || !game->map->isPathValid(this, route, mType->canPushItems)) {
            ...
            if (calcMovePosition()) {
                ...
                if (route.empty() && !(state == STATE_IDLESUMMON) &&
                    (!mType->hasDistanceAttack || !validateDistanceAttack(targetPos))) {
...
bool Monster::getDistancePosition(const Position &target, const int &maxTryDist, bool fullPathSearch, Position &dest) {
    ...
    for (int y = target.y - ymindelta; y <= target.y + ymaxdelta; ++y) {
        for (int x = target.x - xmindelta; x <= target.x + xmaxdelta; ++x) {
            ...
            if (tryDist <= (maxTryDist * maxTryDist) && (tryDist >= prevDist) &&
                (tryDist > (minTryDist * minTryDist))) {
    ...
    if (positionList.empty()) {
        dest = this->pos;
        return currentDistance <= maxTryDist &&
               (game->map->canThrowObjectTo(this->pos, target, BLOCK_PROJECTILE) == RET_NOERROR);
    ...
    if (isSummon()) {
        ...
        if (state != STATE_IDLE) {
            if (mType->changeTargetChance > rand() * 10000 / (RAND_MAX + 1)) {
                ...
                if (newtarget && (canReach || state == STATE_TARGETNOTREACHABLE) &&
                    newtarget->getID() != attackedCreature) {
...
void Monster::startThink() {
    ...
    if (attackedCreature != 0 && !(isSummon() && hasLostMaster)) {
        if (!eventCheckAttacking) {
            eventCheckAttacking = game->addEvent(
                    makeTask(500, std::bind2nd(std::mem_fun(&Game::checkCreatureAttacking), getID())));
        }
    }
}
...
bool Monster::doAttacks(Creature *attackedCreature, monstermode_t mode /*= MODE_NORMAL*/) {
    ...
    bool hasmelee = false;
    if (trymelee) {
        for (PhysicalAttacks::iterator paIt = mType->physicalAttacks.begin();
             paIt != mType->physicalAttacks.end(); ++paIt) {
    ...
    if (exhaustedTicks <= 0) {
        ...
        for (InstantAttackSpells::iterator iaIt = mType->instantSpells.begin();
             iaIt != mType->instantSpells.end(); ++iaIt) {
...
void Monster::onAttack() {
    if (attackedCreature != 0 && !(isSummon() && hasLostMaster)) {
        this->eventCheckAttacking = game->addEvent(
                makeTask(500, std::bind2nd(std::mem_fun(&Game::checkCreatureAttacking), getID())));
