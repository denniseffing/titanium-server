...
std::list<Position> Map::getPathTo(Creature *creature, Position start, Position to,
                                   bool creaturesBlock /*=true*/, bool ignoreMoveableBlockingItems /*= false*/,
                                   int maxNodSize /*= 100*/) {
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                if (std::abs(dx) != std::abs(dy)) {
                    ...
                    if (t) {
                        ReturnValue ret = t->isBlocking(BLOCK_SOLID | BLOCK_PATHFIND, !creaturesBlock,
                                                        ignoreMoveableBlockingItems);
                        if (ret == RET_CREATUREBLOCK && t->getCreature() == creature && t->creatures.size() == 1)
                            ret = RET_NOERROR;
