...
void MonsterType::reset() {
    ...
    for (std::map<PhysicalAttackClass *, TimeProbabilityClass>::iterator it = physicalAttacks.begin();
         it != physicalAttacks.end(); ++it) {
        delete it->first;
    }
...
MonsterType *
Monsters::loadMonster(const std::string &file, const std::string &monster_name, bool reloading /*= false*/) {
    bool monsterLoad;
    MonsterType *mType;
    bool new_mType = true;
                ...
                while (tmp) {
                    if (strcmp((const char *) tmp->name, "attack") == 0) {
                        ...
                        if (strcmp(attacktype.c_str(), "melee") == 0) {
                            ...
                            mType->physicalAttacks[physicalattack] = TimeProbabilityClass(cycleTicks, probability,
                                                                                          exhaustionTicks);
                            ...
                            std::map<uint16_t, Spell *>::const_iterator rsIt;
                            for (rsIt = spells.getAllRuneSpells()->begin();
                                 rsIt != spells.getAllRuneSpells()->end(); ++rsIt) {
                ...
                while (tmp) {
                    if (strcmp((const char *) tmp->name, "voice") == 0) {
                        ...
                        if (sentence.length() > 0) {
                            mType->yellingSentences.push_back(make_pair(sentence,
                                                                        TimeProbabilityClass(cycleTicks, probability,
                                                                                             exhaustionTicks)));
                        }
                    }
                    tmp = tmp->next;
