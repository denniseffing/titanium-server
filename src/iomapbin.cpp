...
bool IOMapBin::loadMap(Map *map, std::string identifier) {
    ...
    if (revminor > 1 || revnum > 2)
        std::cout
                << "WARNING: This is a newer map revision format. Check for the lastest editors to get the new functions"
                << std::endl;
    ...
    while (!feof(fh) || !end) {
        ...
        switch (op) {
            ...
            case 0x30: // Global Temple Position
            {
                ...
                std::cout << ":: Global Temple Position: " << templePos.x << " " << templePos.y << " " << templePos.z
