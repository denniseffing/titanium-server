...
const int Player::promotedGainManaVector[5][2] = {{6, 1},
                                                  {2, 1},
                                                  {2, 1},
                                                  {3, 1},
                                                  {6, 1}};
const int Player::promotedGainHealthVector[5][2] = {{6, 1},
                                                    {6, 1},
                                                    {6, 1},
                                                    {3, 1},
                                                    {2, 1}};
...
const int Player::gainManaVector[5][2] = {{6, 1},
                                          {3, 1},
                                          {3, 1},
                                          {4, 1},
                                          {6, 1}};
const int Player::gainHealthVector[5][2] = {{6, 1},
                                            {6, 1},
                                            {6, 1},
                                            {4, 1},
                                            {3, 1}};
            ...
            if ((items[slot]->isWeapon())) {
                ...
                switch (items[slot]->getWeaponType()) {
                    case SWORD:
                        ...
                        damagemax =
                                mul * getSkill(SKILL_SWORD, SKILL_LEVEL) * Item::items[items[slot]->getID()].attack /
                                20 + Item::items[items[slot]->getID()].attack;
                        break;
                    case CLUB:
                        //damagemax = 3*skills[SKILL_CLUB][SKILL_LEVEL] + 2*Item::items[items[slot]->getID()].attack;
                        damagemax = mul * getSkill(SKILL_CLUB, SKILL_LEVEL) * Item::items[items[slot]->getID()].attack /
                                    20 + Item::items[items[slot]->getID()].attack;
                        break;
                    case AXE:
                        //damagemax = 3*skills[SKILL_AXE][SKILL_LEVEL] + 2*Item::items[items[slot]->getID()].attack;
                        damagemax =
                                mul * getSkill(SKILL_AXE, SKILL_LEVEL) * Item::items[items[slot]->getID()].attack / 20 +
                                Item::items[items[slot]->getID()].attack;
                        break;
                    case DIST:
                        ...
                        if (distitem) {
                            ...
                            damagemax =
                                    mul * getSkill(SKILL_DIST, SKILL_LEVEL) * Item::items[distitem->getID()].attack /
                                    20 + Item::items[distitem->getID()].attack;
...
bool Player::removeItem(uint16_t id, long count) {
            ...
            if (items[i]->getID() == id) {
                if (items[i]->isStackable()) {
                    if (items[i]->getItemCountOrSubtype() > count) {
                        items[i]->setItemCountOrSubtype((uint8_t) (items[i]->getItemCountOrSubtype() - count));
                        sendInventory(i);
                        count = 0;
                    } else {
            ...
            if (item->getID() == id) {
                if (item->isStackable()) {
                    if (item->getItemCountOrSubtype() > count) {
                        item->setItemCountOrSubtype((uint8_t) (item->getItemCountOrSubtype() - count));
...
int Player::sendInventory(uint8_t sl_id) {
...
freeslot_t Player::getFreeSlot(Container **container, uint8_t &slot, const Item *item) {
    *container = NULL;
...
bool Player::internalRemoveItemContainer(Container *parent, Item *item, bool test /*=false*/) {
    ...
    for (ContainerList::const_iterator cit = parent->getItems();
        ...
        if (*cit == item) {
            uint8_t slot = parent->getSlotNumberByItem(item);
            if (slot != 0xFF) {
                if (!test) {
...
uint32_t Player::getReqSkillTries(int skill, int level, playervoc_t voc) {
    //first find on cache
    for (int i = 0; i < 2; i++) {
    ...
    SkillAdvanceCache[skill][j].tries = (uint32_t) (SkillBases[skill] *
                                                    pow((float) SkillMultipliers[skill][voc], (float) (level - 11)));
    ...
    for (int slot = SLOT_RIGHT; slot <= SLOT_LEFT; slot++) {
        if (items[slot]) {
            if (items[slot]->isWeapon()) {
                ...
                addSkillTryInternal(
                        skilltry * (skill == 4 ? g_config.DIST_MUL[vocation] : g_config.WEAPON_MUL[vocation]), skill);
#else
...
void Player::addSkillTryInternal(int skilltry, int skill) {
    ...
    if (skills[skill][SKILL_TRIES] >= getReqSkillTries(skill, (skills[skill][SKILL_LEVEL] + 1), vocation)) {
        ...
        //update percent
        int new_percent = (uint32_t) (100 * (skills[skill][SKILL_TRIES]) /
                                      (1. * getReqSkillTries(skill, (skills[skill][SKILL_LEVEL] + 1), vocation)));
...
uint32_t Player::getReqMana(int maglevel, playervoc_t voc) {
    ...
    uint32_t reqMana = (uint32_t) (400 * pow(ManaMultiplier[(int) voc], maglevel - 1));
...
Container *Player::getContainer(uint8_t containerid) {
...
uint8_t Player::getContainerID(const Container *container) const {
...
void Player::addContainer(uint8_t containerid, Container *container) {
#ifdef __DEBUG__
...
void Player::closeContainer(uint8_t containerid) {
    for (containerLayout::iterator cl = vcontainers.begin(); cl != vcontainers.end(); ++cl) {
        if (cl->first == containerid) {
...
void Player::removeDistItem() {
    ...
    if (DistItem) {
        ...
        uint8_t n = (uint8_t) DistItem->getItemCountOrSubtype();
...
void Player::sendStats() {
    ...
    if (lastSentStats.experience != this->experience || lastSentStats.level != this->level)
        level_percent = (uint8_t) (100 * (experience - getExpForLv(level)) /
                                   (1. * getExpForLv(level + 1) - getExpForLv(level)));

    if (lastSentStats.manaspent != this->manaspent || lastSentStats.maglevel != this->maglevel)
        maglevel_percent = (uint8_t) (100 * (manaspent / (1. * getReqMana(maglevel + 1, vocation))));
...
void Player::sendTextMessage(MessageClasses mclass, const char *message, const Position &pos, uint8_t type) const {
...
void Player::sendDistanceShoot(const Position &from, const Position &to, uint8_t type) {
    client->sendDistanceShoot(from, to, type);
}

void Player::sendMagicEffect(const Position &pos, uint8_t type) {
    client->sendMagicEffect(pos, type);
}

void Player::sendAnimatedText(const Position &pos, uint8_t color, std::string text) {
...
void Player::sendContainer(uint8_t index, Container *container) {
...
void Player::onThingMove(const Creature *creature, const Container *fromContainer, uint8_t from_slotid,
                         const Item *fromItem, int oldFromCount, Container *toContainer, uint8_t to_slotid,
                         const Item *toItem, int oldToCount, int count) {
    client->sendThingMove(creature, fromContainer, from_slotid, fromItem, oldFromCount, toContainer, to_slotid, toItem,
                          oldToCount, count);
}
...
#endif //YUR_RINGS_AMULETS
    client->sendThingMove(creature, fromSlot, fromItem, oldFromCount, toContainer, to_slotid, toItem, oldToCount,
                          count);
#ifdef YUR_BOH
...
void Player::onThingMove(const Creature *creature, const Container *fromContainer, uint8_t from_slotid,
                         const Item *fromItem, int oldFromCount, slots_t toSlot, const Item *toItem, int oldToCount,
                         int count) {
#ifdef YUR_RINGS_AMULETS
    if (toSlot == SLOT_RING) {
        const_cast<Item *>(fromItem)->setGlimmer();
...
#endif //YUR_RINGS_AMULETS
    client->sendThingMove(creature, fromContainer, from_slotid, fromItem, oldFromCount, toSlot, toItem, oldToCount,
                          count);
#ifdef YUR_BOH
    if (toSlot == SLOT_FEET)
        checkBoh();
...
void Player::onThingMove(const Creature *creature, const Container *fromContainer, uint8_t from_slotid,
                         ...
                         int oldToCount, int count) {
    client->sendThingMove(creature, fromContainer, from_slotid, fromItem, oldFromCount, toPos, toItem, oldToCount,
                          count);
}
...
void Player::onThingMove(const Creature *creature, const Position &fromPos, int stackpos, const Item *fromItem,
                         int oldFromCount, const Container *toContainer, uint8_t to_slotid, const Item *toItem,
                         int oldToCount, int count) {
    client->sendThingMove(creature, fromPos, stackpos, fromItem, oldFromCount, toContainer, to_slotid, toItem,
                          oldToCount, count);
}
...
void Player::onCreatureDisappear(const Creature *creature, uint8_t stackPos, bool tele /*= false*/) {
...
void Player::onThingDisappear(const Thing *thing, uint8_t stackPos) {
...
void Player::onItemRemoveContainer(const Container *container, const uint8_t slot) {
    client->sendItemRemoveContainer(container, slot);
}

void Player::onItemUpdateContainer(const Container *container, const Item *item, const uint8_t slot) {
    client->sendItemUpdateContainer(container, item, slot);
}

void Player::onCreatureTurn(const Creature *creature, uint8_t stackPos) {
...
void Player::onTeleport(const Creature *creature, const Position *oldPos, uint8_t oldstackpos) {
...
void Player::die() {
    ...
    for (int i = 0; i <= 6; i++) {  //for each skill
        ...
        lostSkillTries = (long) (sumSkillTries * g_config.DIE_PERCENT_SKILL /
                                 100.0);           //player loses 10% of his skill tries
    ...
    //Level Downgrade
    long newLevel = level;
    while ((experience - getLostExperience()) <
           getExpForLv(newLevel)) //0.1f is also used in die().. maybe we make a little function for exp-loss?
    {
        if (newLevel > 1)
            newLevel--;
        else
...
void Player::preSave() {
    if (health <= 0) {
        ...
        while (experience < getExpForLv(level)) {
            ...
            if ((healthmax -= g_config.HP_GAIN[(int) vocation]) <
                0) //This could be avoided with a proper use of unsigend int
                healthmax = 0;

            health = healthmax;

            if ((manamax -= g_config.MANA_GAIN[(int) vocation]) <
                0) //This could be avoided with a proper use of unsigend int
                manamax = 0;

            mana = manamax;

            if ((capacity -= g_config.CAP_GAIN[(int) vocation]) <
                0) //This could be avoided with a proper use of unsigend int
                capacity = 0.0;
        }
    }
    ...
    if (vocation >= 0 && vocation < 5) {
...
#endif //YUR_PREMIUM_PROMOTION
        {
            if (manaTick < gainManaVector[vocation][0])
                return false;
            else if (manaTick < (gainManaVector[vocation][0] - ((gainManaVector[vocation][0] * 25) / 100)) &&
                     items[SLOT_RING] && (items[SLOT_RING]->getID() != ITEM_LIFE_RING_IN_USE ||
                                          items[SLOT_RING]->getID() != ITEM_RING_OF_HEALING_IN_USE))
                return false;
            else if (manaTick < gainManaVector[vocation][0] && items[SLOT_RING] &&
                     (items[SLOT_RING]->getID() != ITEM_LIFE_RING_IN_USE ||
                      items[SLOT_RING]->getID() != ITEM_RING_OF_HEALING_IN_USE))
                return false;
            manaTick = 0;
        ...
        if (promoted) {
            if (healthTick < promotedGainHealthVector[vocation][0])
                return false;
                     ...
                     (promotedGainHealthVector[vocation][0] - ((promotedGainHealthVector[vocation][0] * 25) / 100)) &&
                     items[SLOT_RING] && (items[SLOT_RING]->getID() != ITEM_LIFE_RING_IN_USE ||
                                          items[SLOT_RING]->getID() != ITEM_RING_OF_HEALING_IN_USE))
                return false;
            else if (healthTick < promotedGainHealthVector[vocation][0] && items[SLOT_RING] &&
                     (items[SLOT_RING]->getID() != ITEM_LIFE_RING_IN_USE ||
                      items[SLOT_RING]->getID() != ITEM_RING_OF_HEALING_IN_USE))
                return false;
            healthTick = 0;
...
bool Player::removeCoins(signed long cost) {
    ...
    for (int slot = 0; slot <= 10; slot++) {
        ...
        if (item && cost > 0) {
            Container *container = dynamic_cast<Container *>(item);
            if (item->getID() == ITEM_COINS_GOLD || item->getID() == ITEM_COINS_PLATINUM ||
                item->getID() == ITEM_COINS_CRYSTAL) {
                value = 1;
                ...
                if (item->getItemCountOrSubtype() * value > cost) {
                    ...
                    signed long newcost = -1 * (value * (item->getItemCountOrSubtype() -
                                                         (item->getItemCountOrSubtype() - worth)) - cost);
    ...
    for (int number = 0; number < container->size(); number++) {
        ...
        if (item->getID() == ITEM_COINS_GOLD || item->getID() == ITEM_COINS_PLATINUM ||
            ...
            if (item->getItemCountOrSubtype() * value > cost) {
                ...
                signed long newcost =
                        -1 * (value * (item->getItemCountOrSubtype() - (item->getItemCountOrSubtype() - worth)) - cost);
                if (item->getItemCountOrSubtype() - worth > 0) {
                    item->setItemCountOrSubtype(item->getItemCountOrSubtype() - worth);
...
void Player::payBack(uint32_t cost) {
    ...
    if (cost / 10000 > 0) {
        TLMaddItem(ITEM_COINS_CRYSTAL, (uint8_t) (cost / 10000));
        cost -= 10000 * (cost / 10000);
    }
    if (cost / 100 > 0) {
        TLMaddItem(ITEM_COINS_PLATINUM, (uint8_t) (cost / 100));
        cost -= 100 * (cost / 100);
    }
    if (cost > 0 && cost < 100) {
        TLMaddItem(ITEM_COINS_GOLD, (uint8_t) cost);
    }
}

void Player::TLMaddItem(int itemid, uint8_t count) {
...
void Player::checkAfk(int thinkTicks) {
    ...
    if (idleTime < g_config.KICK_TIME && idleTime > (g_config.KICK_TIME - 60000) &&
        !warned)    // send warning for kick (1 min)
    {
...
void Player::sendWorldLightLevel(uint8_t lightlevel, uint8_t color) {
