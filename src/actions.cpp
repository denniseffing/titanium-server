...
bool Actions::UseItem(Player *player, const Position &pos, const uint8_t stack,
    ...
    if (Item::items[itemid].isDoor) {
        ...
        if (house && player->access < g_config.ACCESS_HOUSE &&
            house->getPlayerRights(pos, player->getName()) <= HOUSE_GUEST) {
...
bool readXMLInteger(xmlNodePtr p, const char *tag, int &value) {
    ...
    if (!sinteger)
        ...
        uint16_t integer = atoi(sinteger);
        value = integer;
...
int ActionScript::registerFunctions() {
    ...
    //doPlayerMaxHealth(uid,Maxhealth)
    lua_register(luaState, "doPlayerAddMaxHealth", ActionScript::luaActionDoPlayerAddMaxHealth);

    //doPlayerAddMana(uid,mana)
    lua_register(luaState, "doPlayerAddMana", ActionScript::luaActionDoPlayerAddMana);

    //doPlayerAddMaxMana(uid,mana)
    lua_register(luaState, "doPlayerAddMaxMana", ActionScript::luaActionDoPlayerAddMaxMana);
    //doPlayerAddMaxCap(uid,mana)
    lua_register(luaState, "doPlayerAddMaxCap", ActionScript::luaActionDoPlayerAddMaxCap);
...
int ActionScript::luaActionDoRemoveItem(lua_State *L) {
    //doRemoveItem(uid,n)
    char n = (uint8_t) internalGetNumber(L);
...
int ActionScript::luaActionDoPlayerRemoveItem(lua_State *L) {
    //doPlayerRemoveItem(cid,itemid,count)
    long count = (uint8_t) internalGetNumber(L);
    uint16_t itemid = (uint16_t) internalGetNumber(L);
    uint32_t cid = (uint32_t) internalGetNumber(L);
...
int ActionScript::luaActionDoPlayerAddMaxHealth(lua_State *L) {
    //doPlayerAddMaxHealth(uid,Maxhealth)
    int addmaxhealth = (int) internalGetNumber(L);
    uint32_t cid = (uint32_t) internalGetNumber(L);
    ...
    if (tmp) {
        Player *player = (Player *) (tmp->thing);
        int tmp = player->healthmax = addmaxhealth;
        if (tmp <= 0) {
            player->healthmax = 1;
        } else {
            player->health = tmp;
...
//titans
int ActionScript::luaActionDoPlayerAddMaxMana(lua_State *L) {
    //doPlayerAddMaxMana(uid,maxmana)
    int addmaxmana = (int) internalGetNumber(L);
    uint32_t cid = (uint32_t) internalGetNumber(L);
...
int ActionScript::luaActionDoPlayerAddMaxCap(lua_State *L) {
    //doPlayerAddMaxCap(uid,maxmana)
    int addmaxcap = (int) internalGetNumber(L);
    uint32_t cid = (uint32_t) internalGetNumber(L);
    ...
    if (tmp) {
        Player *player = (Player *) (tmp->thing);
        player->capacity = addmaxcap;
        player->sendStats();
    } else {
        lua_pushnumber(L, -1);
        std::cout << "luaDoPlayerAddCap: player not found" << std::endl;
        return 1;
    }
...
int ActionScript::luaActionDoPlayerAddMana(lua_State *L) {
    ...
    if (tmp) {
        ...
        lua_pushnumber(L, -1);
        std::cout << "luaDoPlayerAddMaxMana: player not found" << std::endl;
        return 1;
    }
...
int ActionScript::luaActionDoPlayerSendTextMessage(lua_State *L) {
    //doPlayerSendTextMessage(uid,MessageClasses,message)
    const char *text = internalGetString(L);
    uint8_t messageClass = (uint8_t) internalGetNumber(L);
    uint32_t cid = (uint32_t) internalGetNumber(L);
...
int ActionScript::luaActionGetPlayerSkill(lua_State *L) {
    //getPlayerSkill(uid,skillid)
    uint8_t skillid = (uint32_t) internalGetNumber(L);
    uint32_t cid = (uint32_t) internalGetNumber(L);
...
int ActionScript::luaActionSetPlayerStorageValue(lua_State *L) {
    ...
    long value = (uint32_t) internalGetNumber(L);
    uint32_t key = (uint32_t) internalGetNumber(L);
    uint32_t cid = (uint32_t) internalGetNumber(L);
