...
bool IOPlayerXML::loadPlayer(Player *player, std::string name) {
    ...
    if (doc) {
        ...
        //level percent
        player->level_percent = (uint8_t) (100 * (player->experience - player->getExpForLv(player->level)) /
                                           (1. * player->getExpForLv(player->level + 1) -
                                            player->getExpForLv(player->level)));
        while (p) {
            ...
            if (str == "mana") {
                ...
                player->maglevel_percent = (uint8_t) (100 * (player->manaspent / (1. * player->getReqMana(
                        player->maglevel + 1, player->vocation))));
            } else if (str == "health") {
                nodeValue = (char *) xmlGetProp(p, (const xmlChar *) "now");
                if (nodeValue) {
                ...
                while (tmp) {
                    ...
                    if (strcmp((const char *) tmp->name, "skill") == 0) {
                        ...
                        player->skills[s_id][SKILL_PERCENT] = (uint32_t) (100 * (player->skills[s_id][SKILL_TRIES]) /
                                                                          (1. * player->getReqSkillTries(s_id,
                                                                                                         (player->skills[s_id][SKILL_LEVEL] +
                                                                                                          1),
                                                                                                         player->vocation)));
                    }
                    tmp = tmp->next;
...
bool IOPlayerXML::savePlayer(Player *player) {
    ...
    sb.str("");
    sb << (long) player->lastlogin;
    xmlSetProp(root, (const xmlChar *) "lastlogin", (const xmlChar *) sb.str().c_str());
    sb.str("");

#ifdef YUR_PREMIUM_PROMOTION
    sb << player->premiumTicks;
    xmlSetProp(root, (const xmlChar *) "premticks", (const xmlChar *) sb.str().c_str());
    sb.str("");
    sb << player->promoted;
    xmlSetProp(root, (const xmlChar *) "promoted", (const xmlChar *) sb.str().c_str());
    sb.str("");
#endif //YUR_PREMIUM_PROMOTION
#ifdef TLM_SKULLS_PARTY
    sb << player->banned;
    xmlSetProp(root, (const xmlChar *) "banned", (const xmlChar *) sb.str().c_str());
    sb.str("");
#endif //TLM_SKULLS_PARTY
    ...
    sb << (int) player->skullType;
    xmlSetProp(pn, (const xmlChar *) "type", (const xmlChar *) sb.str().c_str());
    sb.str("");
    sb << player->skullKills;
    xmlSetProp(pn, (const xmlChar *) "kills", (const xmlChar *) sb.str().c_str());
    sb.str("");
    sb << player->skullTicks;
    xmlSetProp(pn, (const xmlChar *) "ticks", (const xmlChar *) sb.str().c_str());
    sb.str("");
    sb << player->absolveTicks;
    xmlSetProp(pn, (const xmlChar *) "absolve", (const xmlChar *) sb.str().c_str());
    sb.str("");
    xmlAddChild(root, pn);
#endif //TLM_SKULLS_PARTY
...
xmlAddChild(root, pn);
    ...
    for (StorageMap::const_iterator cit = player->getStorageIteratorBegin();
         cit != player->getStorageIteratorEnd(); cit++) {
