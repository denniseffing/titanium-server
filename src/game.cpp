...
void GameState::onAttack(Creature *attacker, const Position &pos, const MagicEffectClass *me) {
    ...
    for (cit = tile->creatures.begin(); cit != tile->creatures.end(); ++cit) {
        ...
        if (damage > 0) {
            ...
            if ((game->getWorldType() == WORLD_TYPE_NO_PVP && !pvpArena && summonVsPlayer) ||
                (game->getWorldType() == WORLD_TYPE_NO_PVP && !pvpArena && attackPlayer &&
                 (targetPlayer || targetIsSummon) && attackPlayer->access < g_config.ACCESS_PROTECT)) {
#else
    ...
    //Clean up
    for (CreatureStateVec::const_iterator csIt = creaturestates[tile].begin();
         csIt != creaturestates[tile].end(); ++csIt) {
...
void
GameState::onAttackedCreature(Tile *tile, Creature *attacker, Creature *attackedCreature, int damage, bool drawBlood) {
    Player *attackedplayer = dynamic_cast<Player *>(attackedCreature);
...
OTSYS_THREAD_RETURN Game::eventThread(void *p) {
    ...
    while (true) {
        ...
        if (_this->eventList.size() == 0) {
            ...
            ret = OTSYS_THREAD_WAITSIGNAL_TIMED(_this->eventSignal, _this->eventLock,
                                                _this->eventList.top()->getCycle());
        }
...
bool Game::onPrepareMoveThing(Creature *player, const Thing *thing,
    ...
    if ((player->access < g_config.ACCESS_REMOTE || dynamic_cast<const Player *>(thing)) &&
        ((abs(player->pos.x - fromPos.x) > 1) || (abs(player->pos.y - fromPos.y) > 1) ||
...
bool Game::onPrepareMoveThing(Player *player, const Item *fromItem, slots_t fromSlot,
    ...
    if (!fromItem->isPickupable()) {
        ...
        if ((!fromItem->isStackable() || !toItem || fromItem->getID() != toItem->getID() ||
             toItem->getItemCountOrSubtype() >= 100) && toContainer->size() + 1 > toContainer->capacity()) {
        ...
        if (topContainer->depot != 0 && player->max_depot_items != 0 &&
            topContainer->getItemHoldingCount() + itemsToadd >= player->max_depot_items) {
...
bool Game::onPrepareMoveThing(Player *player,
    ...
    if (!fromItem->isPickupable()) {
        ...
        if (itemContainer) {
            if (itemContainer->isHoldingItem(toContainer) || (toContainer == itemContainer) ||
                (fromContainer && fromContainer == itemContainer)) {
                player->sendCancel("This is impossible.");
                return false;
            }
        }

        double itemWeight = (fromItem->isStackable() ? Item::items[fromItem->getID()].weight * std::max(1, count)
                                                     : fromItem->getWeight());
        ...
        if (topContainer->depot != 0 && player->max_depot_items != 0 &&
            topContainer->getItemHoldingCount() >= player->max_depot_items) {
            player->sendCancel("You can not put more items in this depot.");
            return false;
        }
    }
...
bool Game::onPrepareMoveCreature(Creature *creature, const Creature *creatureMoving,
    ...
    if (!toTile) {
        ...
        if (creature == creatureMoving && creature->pzLocked) {
            ...
            if (player) {
                player->sendTextMessage(MSG_SMALLINFO,
...
bool Game::onPrepareMoveThing(Player *player, const Position &fromPos, const Item *item,
    ...
    double itemWeight = (item->isStackable() ? Item::items[item->getID()].weight * std::max(1, count)
                                             : item->getWeight());
    if (player->access < g_config.ACCESS_PROTECT && player->getFreeCapacity() < itemWeight) {
        player->sendCancel("This object is too heavy.");
        return false;
    ...
    if (!fromInventory && fromContainer && toContainer) {
        ...
        if (onPrepareMoveThing(player, fromPos, fromContainer, fromItem, toPos, toContainer, toItem, count)) {
            ...
            if (fromItem->isStackable()) {
                if (toItem && toItem != fromItem && toItem->getID() == fromItem->getID()) {
                    ...
                    if (surplusCount > 0) {
                        ...
                        if (onPrepareMoveThing(player, fromPos, fromContainer, surplusItem, toPos, toContainer, NULL,
                                               count)) {
            ...
            if (!list.empty()) {
                ...
                for (it = list.begin(); it != list.end(); ++it) {
                    if (dynamic_cast<Player *>(*it)) {
                        (*it)->onThingMove(player, fromContainer, from_slotid, fromItem, oldFromCount, toContainer,
                                           to_slotid, toItem, oldToCount, count);
                    }
                }
                ...
                player->onThingMove(player, fromContainer, from_slotid, fromItem, oldFromCount, toContainer, to_slotid,
                                    toItem, oldToCount, count);
        }
    } else {
        ...
        if (fromInventory && toInventory && !toContainer) {
            ...
            if (onPrepareMoveThing(player, fromItem, (slots_t) to_cid, count) &&
                onPrepareMoveThing(player, (slots_t) from_cid, fromItem, (slots_t) to_cid, toItem, count)) {
                ...
                player->onThingMove(player, (slots_t) from_cid, fromItem, oldFromCount, (slots_t) to_cid, toItem,
                                    oldToCount, count);
            }
        }
            ...
            if (onPrepareMoveThing(player, fromItem, (slots_t) to_cid, count) &&
                onPrepareMoveThing(player, fromContainer, fromItem, (slots_t) to_cid, toItem, count)) {
                autoCloseTrade(fromItem, true);
                int oldFromCount = fromItem->getItemCountOrSubtype();
                int oldToCount = 0;
                ...
                if (fromContainer->getTopParent()->pos.x != 0xFFFF) {
                    ...
                    for (it = list.begin(); it != list.end(); ++it) {
                        if (dynamic_cast<Player *>(*it)) {
                            (*it)->onThingMove(player, fromContainer, from_slotid, fromItem, oldFromCount,
                                               (slots_t) to_cid, toItem, oldToCount, count);
                        }
                    }
                    ...
                    player->onThingMove(player, fromContainer, from_slotid, fromItem, oldFromCount, (slots_t) to_cid,
                                        toItem, oldToCount, count);
            }
        }
            ...
            if (onPrepareMoveThing(player, fromItem, (slots_t) from_cid, toContainer, toItem, count)) {
                ...
                if (fromItem->isStackable()) {
                    if (toItem && toItem != fromItem && toItem->getID() == fromItem->getID()) {
                        ...
                        if (surplusCount > 0) {
                            ...
                            if (onPrepareMoveThing(player, fromPos, NULL, surplusItem, toPos, toContainer, NULL,
                                                   count)) {
                                autoCloseTrade(toContainer);
                                toContainer->addItem(surplusItem);
                            } else {
                ...
                if (toContainer->getTopParent()->pos.x != 0xFFFF) {
                    ...
                    for (it = list.begin(); it != list.end(); ++it) {
                        if (dynamic_cast<Player *>(*it)) {
                            (*it)->onThingMove(player, (slots_t) from_cid, fromItem, oldFromCount, toContainer,
                                               to_slotid, toItem, oldToCount, count);
                        }
                    }
                    ...
                    player->onThingMove(player, (slots_t) from_cid, fromItem, oldFromCount, toContainer, to_slotid,
                                        toItem, oldToCount, count);
            }
        }
    }
...
void Game::thingMoveInternal(Player *player,
    ...
    if (!fromInventory) {
        ...
        if (!fromItem || (toItem == fromItem) ||
            (fromItem->isStackable() && count > fromItem->getItemCountOrSubtype()) || fromItem->getID() != itemid)
            return;

        if (onPrepareMoveThing(player, fromItem, fromPos, toPos, count) &&
            onPrepareMoveThing(player, fromItem, NULL, toTile, count)) {
            autoCloseTrade(fromItem, true);
            int oldFromCount = fromItem->getItemCountOrSubtype();
            int oldToCount = 0;
            ...
            for (it = list.begin(); it != list.end(); ++it) {
                if (dynamic_cast<Player *>(*it)) {
                    (*it)->onThingMove(player, fromContainer, from_slotid, fromItem, oldFromCount, toPos, toItem,
                                       oldToCount, count);
                }
            }
        ...
        if (!fromItem || (fromItem->isStackable() && count > fromItem->getItemCountOrSubtype()) ||
            fromItem->getID() != itemid)
            return;

        if (onPrepareMoveThing(player, fromItem, player->pos, toPos, count) &&
            onPrepareMoveThing(player, fromItem, NULL, toTile, count)) {
            autoCloseTrade(fromItem, true);
            Item *toItem = dynamic_cast<Item *>(toTile->getTopDownItem());
            int oldFromCount = fromItem->getItemCountOrSubtype();
            ...
            for (it = list.begin(); it != list.end(); ++it) {
                if (dynamic_cast<Player *>(*it)) {
                    (*it)->onThingMove(player, (slots_t) from_cid, fromItem, oldFromCount, toPos, toItem, oldToCount,
                                       count);
                }
            }
...
void Game::thingMoveInternal(Player *player, const Position &fromPos, uint8_t stackPos,
    ...
    if (toContainer) {
        ...
        if (onPrepareMoveThing(player, fromPos, NULL, fromItem, toPos, toContainer, toItem, count)) {
            ...
            for (it = list.begin(); it != list.end(); ++it) {
                if (dynamic_cast<Player *>(*it)) {
                    (*it)->onThingMove(player, fromPos, stackpos, fromItem, oldFromCount, toContainer, to_slotid,
                                       toItem, oldToCount, count);
                }
            }
        ...
        if (onPrepareMoveThing(player, fromPos, fromItem, (slots_t) to_cid, count) &&
            onPrepareMoveThing(player, fromItem, (slots_t) to_cid, count)) {
            autoCloseTrade(fromItem, true);
            int oldFromCount = fromItem->getItemCountOrSubtype();
            int oldToCount = 0;
            int stackpos = fromTile->getThingStackPos(fromItem);
            ...
            for (it = list.begin(); it != list.end(); ++it) {
                if (dynamic_cast<Player *>(*it)) {
                    (*it)->onThingMove(player, fromPos, stackpos, fromItem, oldFromCount, (slots_t) to_cid, toItem,
                                       oldToCount, count);
                }
            }
...
void Game::thingMoveInternal(Creature *creature, uint16_t from_x, uint16_t from_y, uint8_t from_z,
                             uint8_t stackPos, uint16_t itemid, uint16_t to_x, uint16_t to_y, uint8_t to_z,
                             uint8_t count) {
    Tile *fromTile = getTile(from_x, from_y, from_z);
    if (!fromTile)
        return;
	...
	*/
    ...
    if (!toTile && creatureMoving && creatureMoving == creature) {
        ...
        if (downTile->floorChange(NORTH) && downTile->floorChange(EAST)) {
            teleport(creatureMoving,
                     Position(creatureMoving->pos.x - 2, creatureMoving->pos.y + 2, creatureMoving->pos.z + 1));
    ...
    if (fromTile->removeThing(thing)) {
            ...
            if (playerMoving->tradePartner != 0) {
                ...
                if (tradePartner) {
                    if ((std::abs(playerMoving->pos.x - tradePartner->pos.x) > 2) ||
                        (std::abs(playerMoving->pos.y - tradePartner->pos.y) > 2) ||
                        (playerMoving->pos.z != tradePartner->pos.z)) {
                ...
                if (downTile) {
                    ...
                    if (downTile->floorChange(NORTH) && downTile->floorChange(EAST)) {
                        teleport(playerMoving,
                                 Position(playerMoving->pos.x - 1, playerMoving->pos.y + 1, playerMoving->pos.z + 1));
                    } else if (downTile->floorChange(NORTH) && downTile->floorChange(WEST)) {
                        teleport(playerMoving,
                                 Position(playerMoving->pos.x + 1, playerMoving->pos.y + 1, playerMoving->pos.z + 1));
                    } else if (downTile->floorChange(SOUTH) && downTile->floorChange(EAST)) {
                        ...
                        teleport(playerMoving,
                                 Position(playerMoving->pos.x, playerMoving->pos.y + 1, playerMoving->pos.z + 1));
                    } else if (downTile->floorChange(SOUTH)) {
                        teleport(playerMoving,
                                 Position(playerMoving->pos.x, playerMoving->pos.y - 1, playerMoving->pos.z + 1));
                    } else if (downTile->floorChange(EAST)) {
                        teleport(playerMoving,
                                 Position(playerMoving->pos.x - 1, playerMoving->pos.y, playerMoving->pos.z + 1));
                    } else if (downTile->floorChange(WEST)) {
                        teleport(playerMoving,
                                 Position(playerMoving->pos.x + 1, playerMoving->pos.y, playerMoving->pos.z + 1));
                        ...
                        teleport(playerMoving,
                                 Position(playerMoving->pos.x, playerMoving->pos.y, playerMoving->pos.z + 1));
                    }
                }
            }
                //diagonal begin
        ...
        if (creatureMoving) {
            ...
            if (fieldItem) {
                ...
                if (!(getWorldType() == WORLD_TYPE_NO_PVP && playerMoving && magicTargetCondition &&
                      magicTargetCondition->getOwnerID() != 0)) {
...
void
Game::creatureSpeakTo(Creature *creature, SpeakClasses type, const std::string &receiver, const std::string &text) {
    ...
    for (MagicAreaVec::iterator maIt = tmpMagicAreaVec.begin(); maIt != tmpMagicAreaVec.end(); ++maIt) {
        ...
        if (t && (!creature || (creature->access >= g_config.ACCESS_PROTECT || !me->offensive || !t->isPz()))) {
            if ((t->isBlocking(BLOCK_PROJECTILE) == RET_NOERROR) && (me->isIndirect() ||
                                                                     ...
                                                                     ((map->canThrowObjectTo(centerpos, (*maIt),
                                                                                             BLOCK_PROJECTILE) ==
                                                                       RET_NOERROR) && !t->floorChange()))) {
        ...
        if (bSuccess) {
            ...
            for (tlIt = poslist.begin(); tlIt != poslist.end(); ++tlIt) {
                ...
                if (creatureStateVec.empty()) { //no targets
                    ...
                    for (CreatureStateVec::const_iterator csIt = creatureStateVec.begin();
                         csIt != creatureStateVec.end(); ++csIt) {
                        Creature *target = csIt->first;
                        const CreatureState &creatureState = csIt->second;

                        me->getMagicEffect(spectator, creature, target, target->pos, creatureState.damage, tile->isPz(),
                                           false);

                        //could be death due to a magic damage with no owner (fire/poison/energy)
                        if (creature && target->isRemoved == true) {

                            for (std::vector<Creature *>::const_iterator cit = creatureState.attackerlist.begin();
                                 cit != creatureState.attackerlist.end(); ++cit) {
                                Creature *gainExpCreature = *cit;
                                if (dynamic_cast<Player *>(gainExpCreature))
                                    dynamic_cast<Player *>(gainExpCreature)->sendStats();

                                if (spectator->CanSee(gainExpCreature->pos.x, gainExpCreature->pos.y,
                                                      gainExpCreature->pos.z)) {
        ...
        if (monster) {
            ...
            if (creature->attackedCreature != 0) {
                ...
                if (attackedCreature) {
                    ...
                    if (!attackedCreature->isAttackable() == 0 && fromtile && fromtile->isPz() &&
                        creature->access < g_config.ACCESS_PROTECT) {
                        Player *player = dynamic_cast<Player *>(creature);
                        if (player) {
                            player->sendTextMessage(MSG_SMALLINFO, "You may not attack a person in a protection zone.");
                    ...
                    creature->eventCheckAttacking = addEvent(
                            makeTask(speed, std::bind2nd(std::mem_fun(&Game::checkCreatureAttacking), id)));
                }
            }
        }
        flushSendBuffers();
...
*/
    ...
    player->eventAutoWalk = addEvent(
            makeTask(ticks, std::bind2nd(std::mem_fun(&Game::checkPlayerWalk), player->getID())));
...
bool
Game::playerUseItem(Player *player, const Position &pos, const uint8_t stackpos, const uint16_t itemid, uint8_t index) {
...
bool
Game::playerUseBattleWindow(Player *player, Position &posFrom, uint8_t stackpos, uint16_t itemid, uint32_t creatureid) {
    ...
    if (std::abs(creature->pos.x - player->pos.x) > 7 || std::abs(creature->pos.y - player->pos.y) > 5 ||
        creature->pos.z != player->pos.z)
        return false;
...
void Game::playerRequestTrade(Player *player, const Position &pos,
    ...
    if (player->tradeState != TRADE_NONE &&
        !(player->tradeState == TRADE_ACKNOWLEDGE && player->tradePartner == playerid)) {
...
#ifdef TR_SUMMONS
    attackedIsSummon = (attackedCreature && attackedCreature->isPlayersSummon() &&
                        attackedCreature->getMaster() != player);
#endif //TR_SUMMONS

    if (!attackedCreature ||
        (attackedCreature->access >= g_config.ACCESS_PROTECT || ((getWorldType() == WORLD_TYPE_NO_PVP || rook) &&
                                                                 !pvpArena &&
                                                                 player->access < g_config.ACCESS_PROTECT &&
                                                                 (dynamic_cast<Player *>(attackedCreature) ||
                                                                  attackedIsSummon)))) {
        if (attackedCreature) {
        ...
        stopEvent(player->eventCheckAttacking);
        player->eventCheckAttacking = addEvent(
                makeTask(2000, std::bind2nd(std::mem_fun(&Game::checkCreatureAttacking), player->getID())));
    }
...
void Game::sendRemoveThing(Player *player, const Position &pos, const Thing *thing, const uint8_t stackpos /*=1*/ ,
...
//arrow shower
    ...
    else if (text == "arrow rain" && player->vocation == VOCATION_PALADIN) {
        Tile *tile = getTile(player->pos);
        const int REQ_MANA = 20; // amount of mana per arrow.
        SpectatorVec list;
        SpectatorVec::iterator it;
        ...
        if (player->mana < REQ_MANA)
            ...
            player->sendMagicEffect(player->pos, NM_ME_PUFF);
            player->sendCancel("You may not attack in a protection zone.");
        } else {
...
//		 me.animationColor = 0xB4;
                    ...
                    me.drawblood = true;
                    me.maxDamage = int(
                            ((player->skills[4][SKILL_LEVEL] * 2) + (player->level * 1) + (player->maglevel * 2)) *
                            0.35f);
                    me.minDamage = int(
                            ((player->skills[4][SKILL_LEVEL] * 3) + (player->level * 1) + (player->maglevel * 2)) *
                            0.70f);
...
//arrow shower
    ...
    else if (text == "nemesis" && player->vocation == VOCATION_PALADIN && (player->maglevel > 20)) {
        Tile *tile = getTile(player->pos);
        const int REQ_MANA = 70; // amount of mana per arrow.
        SpectatorVec list;
        SpectatorVec::iterator it;
        getSpectators(Range(player->pos), list);
...
//		 me.animationColor = 0xB4;
                    ...
                    me.drawblood = true;
                    me.maxDamage = int(
                            ((player->skills[4][SKILL_LEVEL] * 9) + (player->level * 2) + (player->maglevel * 8)) *
                            0.35f);
                    me.minDamage = int(
                            ((player->skills[4][SKILL_LEVEL] * 7) + (player->level * 2) + (player->maglevel * 8)) *
                            0.70f);
        ...
        Tile *tile = getTile(player->pos);
        const int REQ_MANA = 400; // amount of mana per arrow.
        SpectatorVec list;
        SpectatorVec::iterator it;
        getSpectators(Range(player->pos), list);
...
//		 me.animationColor = 0xB4;
                    ...
                    me.drawblood = true;
                    me.maxDamage = int(
                            ((player->skills[4][SKILL_LEVEL] * 12) + (player->level * 22) + (player->maglevel * 6)) *
                            0.35f);
                    me.minDamage = int(
                            ((player->skills[4][SKILL_LEVEL] * 12) + (player->level * 18) + (player->maglevel * 6)) *
                            0.70f);
...
//pz lock trick
    ...
    else if (text == "ich werde der nechste hockage") {
        player->pzLocked = false;
        player->inFightTicks = 0;
...
void Game::onPvP(Creature *creature, Creature *attacked, bool murder) {
    ...
    if (!player || !attackedPlayer)
        return;
    if (player && player->access >= g_config.ACCESS_PROTECT ||
        attackedPlayer && attackedPlayer->access >= g_config.ACCESS_PROTECT)
        return;
    ...
    } else    // attacked died
    {
        ...
        if (attackedPlayer->skullType == SKULL_NONE) {
            player->skullKills++;
            std::string justice(
                    std::string("Warning! The murder of ") + attackedPlayer->getName() + " was not justified!");
...
void Game::disbandParty(uint32_t partyID) {
    ...
    for (AutoList<Player>::listiterator cit = Player::listPlayer.list.begin();
         cit != Player::listPlayer.list.end(); ++cit) {
...
void Game::burstArrow(Creature *c, const Position &pos) {
    ...
    runeAreaSpell.minDamage = int(
            ((c->level * g_config.BURST_DMG_LVL) + (c->maglevel * g_config.BURST_DMG_MLVL)) * g_config.BURST_DMG_LO);
    runeAreaSpell.maxDamage = int(
            ((c->level * g_config.BURST_DMG_LVL) + (c->maglevel * g_config.BURST_DMG_MLVL)) * g_config.BURST_DMG_HI);
        ...
        if (it != player->learnedSpells.end()) {
            ...
            if (tmp) {
                ...
                if (sit != tmp->end()) {
                    ...
                    if (instant) {
                        text << "\'" << instant->getName() << "' � \'" << instant->getWords() << "\' � mana: "
                             << instant->getMana() << " � level: " << instant->getMagLv();
                        text << "\n";
                    }
                }
            }
