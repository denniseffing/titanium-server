...
std::string MakeRejectMessage(int place) {
    if (place < 0)
        ...
        msg << "\n Your Charakter is buged or damaged msg Titan.";
        return msg.str();
    }
}
    ...
    if (msg.ReadFromSocket(s)) {
        ...
        if (protId == 0x0201) {
            ...
            if (isclientBanished(s)) {
                msg.AddByte(0x0A);
                msg.AddString("Your IP is banished! OWNED");
            } else {
                //char accstring[16];
                ...
                if (account.accnumber == accnumber &&
                    passwordTest(password, account.password)) // seems to be a successful load
                {
                    msg.AddByte(0x14);
                    ...
                    for (it = account.charList.begin(); it != account.charList.end(); it++) {
                        msg.AddString((*it));
                        msg.AddString(g_config.getGlobalString("worldname", "Gardia"));
                        msg.AddU32(serverip);
                ...
                if (IOAccount::instance()->getPassword(accnumber, name, acc_pass) && passwordTest(password, acc_pass)) {
                    ...
                    if (player) {
                        ...
                        if (player->client->s == 0 && player->isRemoved == false &&
                            !g_config.getGlobalNumber("allowclones", 0)) {
                    ...
                    if (s) {
                        ...
                        if (playerexist && !g_config.getGlobalNumber("allowclones", 0)) {
                            ...
                            msg.Reset();
                            msg.AddByte(0x14);
                            msg.AddString("Your character has been banished! Noob");
                            msg.WriteToSocket(s);
                        }
                            ...
                            msg.Reset();
                            msg.AddByte(0x14);
                            msg.AddString("Server temporarly closed. Come later back");
                            msg.WriteToSocket(s);
                        }
#ifdef YUR_LOGIN_QUEUE
...
int main(int argc, char *argv[]) {
    ...
    std::cout << ":: TiTaNiUm 1 m log" << std::endl;
    std::cout << ":: ~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
...
//	LOG_MESSAGE("main", EVENT, 1, "Starting server");
    ...
    while (g_game.getGameState() != GAME_STATE_SHUTDOWN && listen_errors < 100) {
        ...
        std::cout << "[done]" << std::endl << ":: " << g_config.getGlobalString("servername") << " Server Running..."
                  << std::endl;
        accept_errors = 0;
